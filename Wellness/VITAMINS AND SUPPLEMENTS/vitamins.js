document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchProducts() {
    try {
      showLoadingState();
      // Fetch products by subcategory "Vitamins & Supplements"
      const encodedSubCategory = encodeURIComponent('Vitamins & Supplements');
      ...
      showErrorState('Failed to load Vitamins & Supplements products. Please try again later.');
      products = [];
    }
  }
  ...
  async function fetchProductsByCategory(category) {
    try {
      ...
      // Filter for Vitamins & Supplements products only
      return transformedProducts.filter(product => 
        product.subCategory === 'Vitamins & Supplements' || 
        (product.productSubCategory && product.productSubCategory.toLowerCase().includes('vitamins & supplements'))
      );
  ...
  function showLoadingState() {
    ...
    productGrid.innerHTML = `
      <div class="col-span-full flex justify-center items-center py-8">
        ...
        <span class="ml-3 text-gray-600">Loading Vitamins & Supplements products...</span>
      </div>
    `;
  }
  ...
  function displayProducts(list) {
    ...
    if (list.length === 0) {
      productGrid.innerHTML = `
        <div class="col-span-full text-center py-8">
          <p class="text-gray-500">No Vitamins & Supplements products found.</p>
