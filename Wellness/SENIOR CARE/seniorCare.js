...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchProducts() {
    try {
      showLoadingState();
      // Fetch products by subcategory "Senior Care"
      const encodedSubCategory = encodeURIComponent('Senior Care');
      ...
      showErrorState('Failed to load Senior Care products. Please try again later.');
      products = [];
    }
  }
  ...
  async function fetchProductsByCategory(category) {
    try {
      ...
      // Filter for Senior Care products only from this category
      return transformedProducts.filter(product =>
        product.subCategory === 'Senior Care' ||
        (product.productSubCategory && product.productSubCategory.toLowerCase().includes('senior care'))
      );
  ...
  function showLoadingState() {
    ...
    productGrid.innerHTML = `
      <div class="col-span-full flex justify-center items-center py-8">
        ...
        <span class="ml-3 text-gray-600">Loading Senior Care products...</span>
      </div>
    `;
  }
  ...
  function displayProducts(list) {
    ...
    if (list.length === 0) {
      productGrid.innerHTML = `
        <div class="col-span-full text-center py-8">
          <p class="text-gray-500">No Senior Care products found.</p>
