...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const API_BASE_URL = 'http://localhost:8083/api/products'; // Backend URL
  ...
  async function fetchProducts() {
    try {
      showLoadingState();
      // Fetch products by subcategory "Skin & Hair Care"
      const encodedSubCategory = encodeURIComponent('Skin & Hair Care');
      ...
      showErrorState('Failed to load Skin & Hair Care products. Please try again later.');
      products = [];
    }
  }
  ...
  async function fetchProductsByCategory(category) {
    try {
      ...
      // Filter for Skin & Hair Care products only from this category
      return transformedProducts.filter(product =>
        product.subCategory === 'Skin & Hair Care' ||
        (product.productSubCategory && product.productSubCategory.toLowerCase().includes('skin & hair care'))
      );
  ...
  function showLoadingState() {
    ...
    productGrid.innerHTML = `
      <div class="col-span-full flex justify-center items-center py-8">
        ...
        <span class="ml-3 text-gray-600">Loading Skin & Hair Care products...</span>
      </div>
    `;
  }
  ...
  function displayProducts(list) {
    ...
    if (list.length === 0) {
      productGrid.innerHTML = `
        <div class="col-span-full text-center py-8">
          <p class="text-gray-500">No Skin & Hair Care products found.</p>
