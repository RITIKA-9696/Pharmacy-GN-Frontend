...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const API_BASE_URL = 'http://localhost:8083/api/products'; // Backend URL from controller
  ...
  async function fetchProducts() {
    try {
      showLoadingState();
      // Fetch products by subcategory "Ayurveda & Herbal Products"
      const encodedSubCategory = encodeURIComponent('Ayurveda & Herbal Products');
      ...
      showErrorState('Failed to load Ayurveda & Herbal Products. Please try again later.');
      products = [];
    }
  }
  ...
  async function fetchProductsByCategory(category) {
    try {
      ...
      // Filter for Ayurveda & Herbal Products only
      return transformedProducts.filter(product =>
        product.subCategory === 'Ayurveda & Herbal Products' ||
        (product.productSubCategory && product.productSubCategory.toLowerCase().includes('ayurveda & herbal products'))
      );
  ...
  function showLoadingState() {
    ...
    productGrid.innerHTML = `
      <div class="col-span-full flex justify-center items-center py-8">
        ...
        <span class="ml-3 text-gray-600">Loading Ayurveda & Herbal Products...</span>
      </div>
    `;
  }
  ...
  function displayProducts(list) {
    ...
    if (list.length === 0) {
      productGrid.innerHTML = `
        <div class="col-span-full text-center py-8">
          <p class="text-gray-500">No Ayurveda & Herbal Products found.</p>
