document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchProducts() {
    try {
      showLoadingState();
      // Fetch products by subcategory "Oral Care"
      const encodedSubCategory = encodeURIComponent('Oral Care');
      ...
      showErrorState('Failed to load Oral Care products. Please try again later.');
      products = [];
    }
  }
  ...
  async function fetchProductsByCategory(category) {
    try {
      ...
      // Filter for Oral Care products only from this category
      return transformedProducts.filter(product =>
        product.subCategory === 'Oral Care' ||
        (product.productSubCategory && product.productSubCategory.toLowerCase().includes('oral care'))
      );
  ...
  function showLoadingState() {
    ...
    productGrid.innerHTML = `
      <div class="col-span-full flex justify-center items-center py-8">
        ...
        <span class="ml-3 text-gray-600">Loading Oral Care products...</span>
      </div>
    `;
  }
  ...
  function displayProducts(list) {
    ...
    if (list.length === 0) {
      productGrid.innerHTML = `
        <div class="col-span-full text-center py-8">
          <p class="text-gray-500">No Oral Care products found.</p>
