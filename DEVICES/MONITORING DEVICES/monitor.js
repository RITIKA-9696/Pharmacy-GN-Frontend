...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchProducts() {
    try {
      showLoadingState();
      // Fetch products by subcategory "Monitoring Devices"
      const encodedSubCategory = encodeURIComponent('Monitoring Devices');
      ...
      showErrorState('Failed to load Monitoring Devices products. Please try again later.');
      products = [];
    }
  }
  ...
  async function fetchProductsByCategory(category) {
    try {
      ...
      // Filter for Monitoring Devices products only from this category
      return transformedProducts.filter(product =>
        product.subCategory === 'Monitoring Devices' ||
        (product.productSubCategory && product.productSubCategory.toLowerCase().includes('monitoring devices'))
      );
    } catch (error) {
  ...
  function showLoadingState() {
    ...
    productGrid.innerHTML = `
      <div class="col-span-full flex justify-center items-center py-8">
        ...
        <span class="ml-3 text-gray-600">Loading Monitoring Devices products...</span>
      </div>
    `;
  }
  ...
  function displayProducts(list) {
    ...
    if (list.length === 0) {
      productGrid.innerHTML = `
        <div class="col-span-full text-center py-8">
          <p class="text-gray-500">No Monitoring Devices products found.</p>
