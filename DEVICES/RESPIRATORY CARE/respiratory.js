...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchProducts() {
    try {
      showLoadingState();
      // Fetch products by subcategory "Respiratory Care"
      const encodedSubCategory = encodeURIComponent('Respiratory Care');
      ...
      showErrorState('Failed to load Respiratory Care products. Please try again later.');
      products = [];
    }
  }
  ...
  async function fetchProductsByCategory(category) {
    try {
      ...
      // Filter for Respiratory Care products only
      return transformedProducts.filter(product =>
        product.subCategory === 'Respiratory Care' ||
        (product.productSubCategory && product.productSubCategory.toLowerCase().includes('respiratory care'))
      );
  ...
  function showLoadingState() {
    ...
    productGrid.innerHTML = `
      <div class="col-span-full flex justify-center items-center py-8">
        ...
        <span class="ml-3 text-gray-600">Loading Respiratory Care products...</span>
      </div>
    `;
  }
  ...
  function displayProducts(list) {
    ...
    if (list.length === 0) {
      productGrid.innerHTML = `
        <div class="col-span-full text-center py-8">
          <p class="text-gray-500">No Respiratory Care products found.</p>
